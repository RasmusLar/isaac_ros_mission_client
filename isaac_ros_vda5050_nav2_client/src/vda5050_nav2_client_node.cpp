...
void Vda5050toNav2ClientNode::ActionResponseCallback(
  ...
  if (!goal) {
    RCLCPP_ERROR(this->get_logger(), "Action %s was rejected",
                 action_id.c_str());
    UpdateActionState(action_state_idx, VDAActionState().FAILED,
                      "Action was rejected");
  } else {
    // vda_action_goal_handles_.push_back(goal);
    RCLCPP_INFO(this->get_logger(), "Action %s was accepted",
                action_id.c_str());
    UpdateActionState(action_state_idx, VDAActionState().RUNNING,
                      "Action was accepted");
  }
}
...
void Vda5050toNav2ClientNode::ActionResultCallback(
    const ResultType &result, const size_t &action_state_idx,
  ...
  case rclcpp_action::ResultCode::SUCCEEDED:
    if (result.result->success) {
      ...
      UpdateActionState(action_state_idx, VDAActionState().FINISHED,
                        description);
    } else {
...
Vda5050toNav2ClientNode::Vda5050toNav2ClientNode(
    ...
    : Node("nav2_client_node", options),
      client_ptr_(rclcpp_action::create_client<NavThroughPoses>(
      ...
      info_sub_(create_subscription<std_msgs::msg::String>(
          ...
          std::bind(&Vda5050toNav2ClientNode::InfoCallback, this,
                    std::placeholders::_1))),
      battery_state_topic_(declare_parameter<std::string>("battery_state_topic",
                                                          "battery_state")),
      ...
      docking_server_enabled_(
          declare_parameter<bool>("docking_server_enabled_", false)),
  ...
  undock_client_ =
      rclcpp_action::create_client<UndockAction>(this, kRobotUndockAction);
  ...
  docking_lifecycle_manager_client_ =
      create_client<nav2_msgs::srv::ManageLifecycleNodes>(
          "lifecycle_manager_docking/manage_nodes",
          rmw_qos_profile_services_default, callback_group_);
  amcl_get_parameters_client_ =
      create_client<rcl_interfaces::srv::GetParameters>(
          "amcl/get_parameters", rmw_qos_profile_services_default,
          callback_group_);
  localization_client_ = create_client<std_srvs::srv::Empty>(
      "/trigger_grid_search_localization", rmw_qos_profile_services_default,
      callback_group_);
  // Pause docking server
  if (docking_server_enabled_) {
    while (!dock_detector_switch_client_->wait_for_service(
        std::chrono::seconds(2))) {
      RCLCPP_INFO(
          get_logger(),
          "Waiting for docking server lifecycle manager service to appear.");
    }
    auto request =
  ...
  for (auto &action_server_name : action_server_names_) {
    RCLCPP_DEBUG(get_logger(), "Action server name: %s",
...
CreateErrorReference(const std::string &reference_key,
...
CreateError(ErrorLevel level, const std::string &error_msg,
...
void Vda5050toNav2ClientNode::execute_order() {
  ...
  if (reached_waypoint_) {
    ...
    if (current_node_action_ <
        current_order_->nodes[current_node_].actions.size()) {
      ...
      if (action_status == VDAActionState().FAILED) {
        RCLCPP_ERROR(
            ...
            agv_state_->action_states[current_action_state_].action_id.c_str());
        RCLCPP_ERROR(get_logger(), "Order '%s' failed.",
                     agv_state_->order_id.c_str());
        return;
      }
    } else {
      ...
      if (next_stop_ >= current_order_->nodes.size()) {
        RCLCPP_INFO(get_logger(), "Order %s is completed.",
...
void Vda5050toNav2ClientNode::StateTimerCallback() {
  ...
  try {
    ...
    RCLCPP_ERROR(this->get_logger(), "Could not get robot position: %s",
                 ex.what());
  }
  if (cancel_action_) {
    CancelOrder();
...
bool Vda5050toNav2ClientNode::RunningOrder() {
  ...
  for (auto &error : agv_state_->errors) {
    if (error.error_type != kValidationError &&
        error.error_type != kOrderUpdateError) {
      return false;
    }
  }
...
void Vda5050toNav2ClientNode::NavigateThroughPoses() {
  ...
  send_goal_options.goal_response_callback =
      std::bind(&Vda5050toNav2ClientNode::NavPoseGoalResponseCallback, this,
                std::placeholders::_1);
  send_goal_options.feedback_callback =
      std::bind(&Vda5050toNav2ClientNode::NavPoseFeedbackCallback, this,
                std::placeholders::_1, std::placeholders::_2);
  send_goal_options.result_callback =
      std::bind(&Vda5050toNav2ClientNode::NavPoseResultCallback, this,
                std::placeholders::_1);
  RCLCPP_INFO(get_logger(), "Sending goal for (x: %f, y: %f, t: %f)",
...
void Vda5050toNav2ClientNode::Vda5050ActionsHandler(
  ...
  if (vda5050_action.action_type == "pause_order") {
    RCLCPP_INFO(this->get_logger(), "Get %s request",
  ...
  if (vda5050_action.action_type == kRobotDockAction) {
    ...
    send_goal_options.result_callback =
        [this,
         action_state_idx](const GoalHandleDockAction::WrappedResult &result) {
          ...
          SendBoolRequest<std_srvs::srv::SetBool>(dock_detector_switch_client_,
                                                  switch_request);
          ...
          server_request->command =
              nav2_msgs::srv::ManageLifecycleNodes_Request::PAUSE;
    ...
    if (trigger_global_localization) {
      ...
      SendServiceRequest<std_srvs::srv::Empty>(localization_client_,
    ...
    if (!SendBoolRequest<nav2_msgs::srv::ManageLifecycleNodes>(
      ...
      UpdateActionState(action_state_idx, VDAActionState().FAILED,
                        "Failed to enable docking server");
      return;
    }

    // Send service request to open switch node
    auto enable_switch_request =
        std::make_shared<std_srvs::srv::SetBool::Request>();
    enable_switch_request->data = true;
    if (!SendBoolRequest<std_srvs::srv::SetBool>(dock_detector_switch_client_,
                                                 enable_switch_request)) {
      RCLCPP_ERROR(this->get_logger(), "Failed to enable switch node");
      UpdateActionState(action_state_idx, VDAActionState().FAILED,
                        "Failed to enable switch node");
      return;
    }

    // Send dock_robot action
    // Get transform_tolerance value
    auto get_param_request =
    ...
    try {
      auto get_param_result =
          SendServiceRequest<rcl_interfaces::srv::GetParameters>(
              amcl_get_parameters_client_, get_param_request);
      transform_tolerance =
          static_cast<int>(ceil(get_param_result->values[0].double_value));
    } catch (...) {
      RCLCPP_ERROR(this->get_logger(), "Failed to get transform_tolerance");
      UpdateActionState(action_state_idx, VDAActionState().FAILED,
                        "Failed to get transform_tolerance");
      return;
    }
  ...
  if (vda5050_action.action_type == kRobotUndockAction) {
    ...
    RCLCPP_INFO(this->get_logger(), "Send %s request",
                vda5050_action.action_type.c_str());
    return;
  }
  ...
  if (action_server_map_.count(vda5050_action.action_type) == 0) {
    RCLCPP_ERROR(this->get_logger(), "Action type %s is not supported",
                 vda5050_action.action_type.c_str());
    UpdateActionState(current_action_state_, VDAActionState().FAILED,
...
void Vda5050toNav2ClientNode::InitAGVState() {
  ...
  RCLCPP_DEBUG(
      this->get_logger(), "Obtained %ld node states and %ld action states",
      agv_state_->node_states.size(), agv_state_->action_states.size());
}

void Vda5050toNav2ClientNode::Vda5050toNav2ClientCallback(
    const vda5050_msgs::msg::Order::ConstSharedPtr msg) {
  RCLCPP_INFO(get_logger(), "Order with order_id %s received",
              msg->order_id.c_str());
  std::lock_guard<std::mutex> lock(order_mutex_);
  bool is_running = RunningOrder();
  if (is_running) {
    if (msg->order_id != current_order_->order_id) {
      RCLCPP_INFO(get_logger(), "One order is running. Order %s is ignored",
                  msg->order_id.c_str());
      vda5050_msgs::msg::Error error = CreateError(
          ErrorLevel::WARNING, "An order is running",
          {CreateErrorReference("orderId", msg->order_id)}, kOrderUpdateError);
...
void Vda5050toNav2ClientNode::BatteryStateCallback(
  ...
  agv_state_->battery_state.charging =
      (msg->power_supply_status == 1) ? true : false;
...
void Vda5050toNav2ClientNode::InstantActionsCallback(
  ...
  RCLCPP_INFO(get_logger(), "Instant actions with header_id %d received",
              msg->header_id);
  std::unique_lock<std::mutex> lock(order_mutex_);

  for (const vda5050_msgs::msg::Action &action : msg->instant_actions) {
    auto it = std::find_if(
        agv_state_->action_states.begin(), agv_state_->action_states.end(),
    ...
    RCLCPP_INFO(this->get_logger(), "Processing action %s of type %s.",
    ...
    } else if (action.action_type == "startTeleop" ||
               action.action_type == "stopTeleop") {
      lock.unlock();
...
void Vda5050toNav2ClientNode::TeleopActionHandler(
    const vda5050_msgs::msg::Action &teleop_action) {
  std::unique_lock<std::mutex> lock(order_mutex_);
  ...
  if (teleop_action.action_type == "startTeleop") {
    ...
    if (nav_goal_handle_) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Start teleop: cancelling current mission node.");
    }
  } else {
...
void Vda5050toNav2ClientNode::UpdateActionStatebyId(
    const std::string &action_id, const std::string &action_status) {
  /*
  Update action status on the current state given action's ID.
  */

  // Get action state from current state
  auto it = std::find_if(agv_state_->action_states.begin(),
  ...
  if (it == agv_state_->action_states.end()) {
    RCLCPP_ERROR(
        ...
        action_id.c_str());
    auto error = vda5050_msgs::msg::Error();
    error = CreateError(ErrorLevel::WARNING,
    ...
    RCLCPP_INFO(this->get_logger(), "Update action %s with state %s ",
...
void Vda5050toNav2ClientNode::NavPoseFeedbackCallback(
  ...
  agv_state_->driving = true;
  while (agv_state_->node_states.size() >
         current_order_->nodes.size() - current_node_ - 1) {
    agv_state_->node_states.erase(agv_state_->node_states.begin());
  }
}
...
void Vda5050toNav2ClientNode::NavPoseResultCallback(
  ...
  case rclcpp_action::ResultCode::SUCCEEDED:
    break;
  case rclcpp_action::ResultCode::ABORTED:
    RCLCPP_ERROR(get_logger(), "Goal was aborted at node_id %s",
    ...
    return;
  case rclcpp_action::ResultCode::CANCELED:
    RCLCPP_ERROR(get_logger(), "Goal was canceled at node_id %s",
                 agv_state_->last_node_id.c_str());
...
void Vda5050toNav2ClientNode::OrderValidErrorCallback(
  ...
  vda5050_msgs::msg::Error error =
      CreateError(ErrorLevel::WARNING, "Malformed order",
                  {CreateErrorReference("Error", msg->data)}, kValidationError);
  agv_state_->errors.push_back(error);
}
void Vda5050toNav2ClientNode::SwitchServiceCallback(
    rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture future,
    const size_t action_state_idx, const DockAction::Goal &goal_msg,
    const rclcpp_action::Client<DockAction>::SendGoalOptions
        &send_goal_options) {
  std::lock_guard<std::mutex> lock(order_mutex_);
...
Vda5050toNav2ClientNode::SendServiceRequest(
  ...
  if (executor_.spin_until_future_complete(result) !=
