...
public:
  ...
  using GoalHandleNavThroughPoses =
      rclcpp_action::ClientGoalHandle<NavThroughPoses>;
  ...
  using GoalHandleMissionAction =
      rclcpp_action::ClientGoalHandle<MissionAction>;
...
private:
  ...
  void UpdateActionState(const size_t &action_state_idx,
  ...
  // and processes it
  void Vda5050toNav2ClientCallback(
  ...
  void NavPoseFeedbackCallback(GoalHandleNavThroughPoses::SharedPtr,
                               const NavThroughPoses::Feedback::ConstSharedPtr);
  // Result callback for NavigateThroughPoses goal message
  void
  NavPoseResultCallback(const GoalHandleNavThroughPoses::WrappedResult &result);
  // Execute order message
  void execute_order();
  // Goal response callback for Action goal message
  template <typename ActionType>
  void ActionResponseCallback(
      const typename rclcpp_action::ClientGoalHandle<ActionType>::SharedPtr
          &goal,
      const size_t &action_state_idx);
  // Result callback for Action goal message
  template <typename ResultType>
  void ActionResultCallback(const ResultType &result,
                            const size_t &action_state_idx,
                            const std::string &description = "");
  void CancelOrder();
  void UpdateActionStatebyId(const std::string &action_id,
                             const std::string &action_status);
  void InstantActionsCallback(
  ...
  SendServiceRequest(typename rclcpp::Client<ServiceT>::SharedPtr client,
                     typename ServiceT::Request::SharedPtr request);

  // Sync service request. Return result->success
  template <typename ServiceT>
  bool SendBoolRequest(typename rclcpp::Client<ServiceT>::SharedPtr client,
                       typename ServiceT::Request::SharedPtr request);
  ...
  std::unordered_map<std::string,
                     rclcpp_action::Client<MissionAction>::SharedPtr>
      action_clients_;
