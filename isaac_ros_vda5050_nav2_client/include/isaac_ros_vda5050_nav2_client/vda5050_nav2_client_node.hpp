...
public:
	...
	explicit Vda5050toNav2ClientNode(rclcpp::NodeOptions const& options);
...
private:
	...
	rclcpp::Subscription<vda5050_msgs::msg::InstantActions>::SharedPtr instant_actions_sub_;
	...
	// Timer callback function to publish a std_msgs/String message containing the
	// order_id
	void OrderIdCallback();
	// Return true if there is currently a running order, otherwise return false.
	// This function is NOT thread-safe, and need to be called in a function that
	// has acquired the `order_mutex_` lock.
	bool RunningOrder();
	...
	void InitAGVState();
	// The callback function when the node receives a vda5050_msgs/Order message
	// and processes it
	void Vda5050toNav2ClientCallback(vda5050_msgs::msg::Order::ConstSharedPtr const msg);
	// The callback function when the node receives a std_msgs/String info message
	// and appends it to the status message that gets published
	void InfoCallback(std_msgs::msg::String::ConstSharedPtr const msg);
	// The callback function when the node receives a sensor_msgs/BatteryState
	// message and processes it into a VDA5050 BatteryState message
	void BatteryStateCallback(sensor_msgs::msg::BatteryState::ConstSharedPtr const msg);
	// The callback function when the node receives a nav_msgs/Odometry message
	...
	void NavPoseFeedbackCallback(
		GoalHandleNavThroughPoses::SharedPtr, NavThroughPoses::Feedback::ConstSharedPtr const);
	// Result callback for NavigateThroughPoses goal message
	void NavPoseResultCallback(GoalHandleNavThroughPoses::WrappedResult const& result);
	...
	void ActionResultCallback(
		ResultType const& result,
		size_t const& action_state_idx,
		std::string const& description = "");
	void CancelOrder();
	void UpdateActionStatebyId(std::string const& action_id, std::string const& action_status);
	void InstantActionsCallback(vda5050_msgs::msg::InstantActions::ConstSharedPtr const msg);
	// Handle teleop instant actions
	void TeleopActionHandler(vda5050_msgs::msg::Action const& teleop_action);
	// The callback function when the node receives an order error message.
	void OrderValidErrorCallback(std_msgs::msg::String::ConstSharedPtr const msg);
	...
	// The length of a period before the client publishes the robot state and
	// mission status messages (in seconds)
	double update_feedback_period_{};
	// The length of a period before the client publishes the order ID (in
	// seconds)
	double update_order_id_period_{};
	// Setting this to true will allow the client to print out more descriptive
	// logs such as sending goals and goal completion
	bool verbose_;
	...
	// The value to populate in the first element of AGVState's field informations
	// so the mission dispatcher can correctly parse the messages from the
	// JsonInfoGenerator
	std::string const info_messages_type_ = "user_info";
	...
	std::unordered_map<std::string, rclcpp_action::Client<MissionAction>::SharedPtr>
		action_clients_;
